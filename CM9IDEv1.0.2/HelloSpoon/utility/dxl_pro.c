...
unsigned short new_update_crc(unsigned short crc_accum, unsigned char *data_blk_ptr, unsigned short data_blk_size)
{
	unsigned short i, j;
			...
			if((gbRxLengthEx != bRxLenEx) && (bRxLenEx != 255)) // after Ver 1.11f
			{
				...
				while(ulCounter++ < RX_TIMEOUT_COUNT2)
				{
					if(gbDXLReadPointerEx != gbDXLWritePointerEx) {
						...
						new_nDelay(NANO_TIME_DELAY);// porting ydh added 20120210.
					}
					//uDelay(0);// porting ydh added
					new_nDelay(NANO_TIME_DELAY);// porting ydh added
...
byte rx_PacketEx(word wRxLength){
	...
	for(bCount = 0; bCount < wRxLength; bCount++)
	{
		...
		{
			new_nDelay(NANO_TIME_DELAY); // porting ydh
			...
			new_uDelay(0); //porting ydh added
		}
		if(bTimeout) break;
	...
	if( gbpTxBufferEx[PKT_ID] != BROADCAST_ID )
	{
		...
		if(bLength > 3) //checking available length.
		{
			...
			if(new_update_crc(0, gbpRxBufferEx, wRxLength-2) == wCheckSum){ // -2 : except CRC16
...
byte tx_PacketEx(byte bID, byte bInstruction, word wParameterLength){
   ...
   // TxDStringC("wPacketLength = ");TxDHex8C(wPacketLength);TxDStringC("\r\n");
    ...
    wCheckSum = new_update_crc(0, gbpTxBufferEx, wPacketLength);  // -2 : except CRC16
